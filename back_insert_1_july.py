...
def assert_seshat_row(Polity, Variable, Value_From, Value_To, Date_From, Date_To, Fact_Type, Value_Note):
    ...
    if scoped:
        ...
        if scoped and (Value_Note == 'disputed' or Value_Note == 'uncertain'):
            disputed = True
            #disp_var = unique_var('v:disputed_var')
            #qv.cast(disputed,'xsd:boolean',disp_var) # cast'ing to xsd:gYear fails to return anything and does not complain
            qv.add_triple(pv_var,'Disputed',disputed)

        if scoped and unknown:
            #unkn_var = unique_var('v:unknown_var')
            #qv.cast(disputed,'xsd:boolean',unkn_var) # cast'ing to xsd:gYear fails to return anything and does not complain
...
if __name__ == "__main__":
    ...
    db_id = "test_seshat_majid" #  this gets its own scm: and doc: world
